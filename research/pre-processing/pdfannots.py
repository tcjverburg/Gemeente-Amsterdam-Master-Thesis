...
Source: https://github.com/0xabu/pdfannots/blob/master/pdfannots.py
"""
...
def prettyprint(annots, outlines, outfile, do_group, sections, wrapcol, infile):
    """
    ...
    def fmtpos(annot):
        ...
        if o:
            return ['%d', '(%s)'] % (annot.page.pageno + 1, o.title)
        else:
            return "%d" % (annot.page.pageno + 1)
    ...
    def printannot(annot, extra=None):
        ...
        msglines = [label , msglines[0][1:-1], infile.name.split('/aktes/')[1]]

        # emit Markdown bullet, wrapped as desired

        # print it!

        with open(save_filename,'a', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(msglines)
...
def main():
    ...
    prettyprint(annots, outlines, args.output, args.group, args.sections, args.wrap, args.input)
    return 0
